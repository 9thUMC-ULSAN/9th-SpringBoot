...
public class ReviewRepositoryCustomImpl implements ReviewRepositoryCustom {
    ...
    private BooleanExpression ratingEq(Integer rating) {
        ...
        if (rating == 5) {
            ...
            // "4점대" (rating=4) -> 4.0 <= score < 5.0
            // "3점대" (rating=3) -> 3.0 <= score < 4.0
            float floor = (float) rating;
            float ceil = floor + 1.0f;
            return review.score.goe(floor).and(review.score.lt(ceil));
        }
